    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case PIVOT_BACK2:
                ...
                break;
            case DRIVE_FORWARD:
                driver.drive = common.driveForward(1000);
                state = state.next();
                break;
            case PIVOT135:
                driver.drive = (alliance == Field.AllianceColor.RED ?
                        common.turnToHeading(reverseOnAlliance(115)) :
                        common.turnToHeading(reverseOnAlliance(-55))); // Turn left and back up vs right and forward
                state = state.next();
                break;
            case DRIVE_DIAGONAL:
                driver.drive = (alliance == Field.AllianceColor.RED ?
                        common.driveForward(1750) :
                        common.driveBackward(1050));
                state = state.next();
                break;
            case PIVOT_TO_FACE:
                driver.drive = (alliance == Field.AllianceColor.RED ?
                        common.turnToHeading(reverseOnAlliance(173)) :
                        common.turnToHeading(reverseOnAlliance(-173)));
                state = state.next();
                break;
            case LOWER_LIFT:
            ...
            case RELEASE_REVERSE:
                ...
                break;
            case RELEASE_TURN:
                driver.drive = common.turnDegrees(reverseOnAlliance(5));
                state = state.next();
                break;
            case DONE:
                // Exit the opmode
